...
#define ASSERT_NEAR(a, b, tol) \
    if (std::abs((a) - (b)) > (tol)) return "Assert near failed: " #a " = " + std::to_string(a) + "   !=   " #b " = " + std::to_string(b);

#define ASSERT_APPROX(a, b, tol) \
    if (std::abs((a) - (b)) / std::max(std::abs(a), std::abs(b)) > (tol)) return "Assert approx failed: " #a " = " + std::to_string(a) + "   !=   " #b " = " + std::to_string(b);

#define ASSERT_APPROX_ARRAY(a, b, size, tol) \
    for (size_t i = 0; i < size; i++) \
        if (std::abs((a[i]) - (b[i])) / std::max(std::abs(a[i]), std::abs(b[i])) > (tol)) return "Assert approx failed: " #a "[" + std::to_string(i) + "] = " + std::to_string(a[i]) + "   !=   " #b "[" + std::to_string(i) + "] = " + std::to_string(b[i]);
