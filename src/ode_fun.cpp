...
// Common variables
    ...
    for (index_t k = 0; k < par->num_species; ++k)
    {
        c[k] = std::exp(c_star[k]) - cpar.epsilon;
        M += c[k];
    }
    for (index_t k = 0; k < par->num_species; ++k)
    {
...
// d/dt c_i
    ...
    for (index_t k = 0; k < par->num_species; ++k)
    {
        c_dot[k] = (this->omega_dot[k] - c[k] * 3.0 * R_dot / R) * cpar.t_ref / (c[k] + cpar.epsilon);
...
// Evaporation
    ...
    if (cpar.enable_evaporation)
    {
        ...
        c_dot[par->index_of_water] += (1.0e-6 * n_net_dot * 3.0 / R) * cpar.t_ref / (c[par->index_of_water] + cpar.epsilon);
    }
